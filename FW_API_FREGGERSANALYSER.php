...
class FW_API_FREGGERSANALYSER {
	public  $data = array(); //Beinhaltet die Daten
	private $analysed = array (0,0,0); //Verhindert das Seiten mehrmals ausgelesen werden
	private $content, //Beinhaltet das zu analysierende Material
			$cache,   //Variable um Daten zwischenspeichern zu können
			$cache2,  //Variable um Daten zwischenspeichern zu können
			$i;       //Variblen für Schleifen
	...
	public function getProfile(){
		if ($this->analysed[0] == 0){
			...
			if ($this->content){
				...
				//Gesamte bisher erhaltene Freekröten
	...
	public function getHP(){
		if ($this->analysed[1] == 0){
			...
			if ($this->content){
				...
				//Alle Abzeichen
				$this->content =preg_replace("/\s+/", " ", $this->content);
				//preg_match_all('|<div class="badge-name">(.*)</div>|siU',$this->content,$this->cache); //Ermöglicht das auslesen der Namen !!!!!
				preg_match_all('|class="ba-badge-container" style="float: left;">(.*)</div> </div> </div>|siU',$this->content,$this->cache); //Version ohne Namen aber dafür fertiige Abzecihen Zähler
	...
	public function getGeschlecht(){
		...
		if ($this->analysed[2] == 0){
			$this->analysed[2] = 1;			
			$this->content = @imagecreatefrompng('http://www.freggers.'.$this->data["SERVER"].'/img/large/'.$this->data["ID"].'/1_idle_0/fregger.png?i=2;i=13;i=33;i=991;');
			echo 'http://www.freggers.'.$this->data["SERVER"].'/img/large/'.$this->data["ID"].'/1_idle_0/fregger.png?i=2;i=13;i=33;i=991;';
			if($this->content){
				$this->data["GESCHLECHT"] = '2'; //Weiblich
				if (imagecolorat ( $this->content , 89 , 89 ) == 0){
					if (imagecolorat ( $this->content , 143 , 156 ) == 0){
...
//$neuerFregger = new FW_API_FREGGERSANALYSER ("freggern","de" , true);
